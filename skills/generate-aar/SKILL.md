...
1. Read the `AAR_PIPELINE_PATH` environment variable — this is the absolute path to the cloned `aar-pipeline` repository on disk.
2. If `AAR_PIPELINE_PATH` is **not set**, stop immediately and tell the user: "The `AAR_PIPELINE_PATH` environment variable is not set. Please set it to the absolute path of the cloned AAR pipeline repository (e.g. `export AAR_PIPELINE_PATH=/path/to/ocap-report-agent`) and try again."
